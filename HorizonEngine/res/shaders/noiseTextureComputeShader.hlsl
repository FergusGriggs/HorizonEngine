...
RWTexture3D<float> bufferOut : register(u0);
...
void CSMain(uint3 id : SV_DispatchThreadID)
{
    ...
    float noiseWidthOffset = snoise(float3(xOffset, yOffset, zOffset) * 0.01f * noiseSize + 1000.0f + seed) * minD;
    float noise = snoise(id.xyz * 0.01f * noiseSize + seed) * (1.0f - maxD);

    bufferOut[id.xyz] = noiseWidthOffset + noise;
}

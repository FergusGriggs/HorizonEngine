...
#define dayZenithDensity(yCoord) cb_density / pow(abs(max(yCoord - cb_zenithOffset, 0.0035f)), 0.75f)
#define nightZenithDensity(yCoord) cb_nightDensity / pow(abs(max(yCoord + 0.05f, cb_nightZenithYClamp)), 0.75f)
...
{
    return 1.0f + pow(abs(1.0f - clamp(radialDistance(viewDirection, sunDirection), 0.0f, 1.0f)), 2.0f) * pi * 0.5f;
}
...
float getMie(float3 viewDirection, float3 sunDirection)
{
    ...
    float disk = clamp(1.0f - pow(abs(radialDistance(viewDirection, sunDirection) * scale), 0.1f), 0.0f, 1.0f);
...
float4 main(VSPS_TRANSFER input) : SV_TARGET
{
    ...
    colour = pow(abs(colour), float3(2.2f, 2.2f, 2.2f)); //Back to linear

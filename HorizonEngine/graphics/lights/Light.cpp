...
bool Light::Initialize(std::string label, ID3D11Device* device, ID3D11DeviceContext* deviceContext, ConstantBuffer<CB_VS_vertexShader>& cb_vs_vertexShader)
{
	this->label = label;

	if (!model.Initialize("res/models/light.fbx", device, deviceContext, cb_vs_vertexShader))
		return false;
